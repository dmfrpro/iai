...
public class Main {
    ...
    record Snapshot(List<Pos> shortestPath, Cells cells, long timeMillis) {
        @Override
        public String toString() {
    ...
    static class Cells implements Cloneable {
        ...
        public Cells clone() {
            try {
                var clone = (Cells) super.clone();

                var clonedCells = new CellType[9][9];
                for (int i = 0; i < 9; i++)
...
//                .sorted((p1, p2) -> p1.squaredDistanceTo(cells.getChest()) - p2.squaredDistanceTo(cells.getChest()))
        ...
        private void takeSnapshot() {
            currentSnapshot = new Snapshot(new ArrayList<>(shortestPath), cells.clone(), 0);
        }

        private void doBacktracking(Pos currentPos) {
            if (isLosingPos(currentPos)) return;

            // +1 for the new pos which will be pushed into steps
